...
describe('User', () => {
  describe('GET', () => {
    test('Deve retornar status NOT_FOUND e sem nenhum dado', async () => {
    ...
    test('Deve retornar status OK e com lista de usuários', async () => {
    ...
    test('Deve retornar status OK e com lista de apenas o usuário buscado', async () => {
    ...
    test.each([
      ...
      '(asd)', '[asd]',
      'asrr d2d', '3aasd]'
    ])('Deve retornar status BAD_REQUEST ao passar query inválida: %s', async (queryParam) => {
      const expected: ExpectType = {
        data: null,
    ...
    test('Deve retornar status OK e apenas o usuário buscado por Id', async () => {
      await userBuilder().insert()
      const user = await userBuilder().insert()
    ...
    test('Deve retornar status NOT_FOUND e erro de entidade não encontrada pelo Id passado', async () => {
  ...
  describe('POST', () => {
    test('Deve criar um usuário e retornar CREATED', async () => {
  ...
  `('Não deve criar usuário e retornar BAD_REQUEST dado o payload: $payload', async ({ payload, message }) => {
      const expected: ExpectType = {
        data: null,
        errors: [message],
      }
  ...
  ${{ name: 'Rodrigo' }}       | ${'body must have required property \'age\''}
  ${{ name: 'Edson', age: -1 }}| ${'body/age must be >= 0'}
  `('Não deve atualizar usuário e retornar BAD_REQUEST dado o payload: $payload', async ({ payload, message }) => {
      const expected: ExpectType = {
        data: null,
        errors: [message],
      }
    ...
    test('Deve atualizar usuário, retornar CREATED e retornar os dados atualizados', async () => {
      const user = await userBuilder().insert()
    ...
    test('Não deve atualizar usuário e retornar BAD_REQUEST', async () => {
